...
// Clickign dustbox icon in "Agenda"
function removeButton(svg_agenda) {
  // prevent running the changePage function at the same time
  event.stopPropagation();
  
  const pageInfoValue = svg_agenda.parentNode.parentNode.dataset.value;
  svg_agenda.parentNode.parentNode.remove(); 
  if (pageInfoValue == document.getElementById('page-info').textContent) { // If the agenda is the same page as current page,
    clearSentence();
    clearTitle();
    updPageInfo();
  }
...
// Cliking "Agenda"
function changePage(div_agenda) {
  const pageInfoValue = div_agenda.dataset.value;
  const hiddenTextId = `hidden-text${pageInfoValue}`;

  if (pageInfoValue != document.getElementById('page-info').textContent) { // If the agenda is differnt from current page,
    const hiddenDivs = Array.from(document.getElementById(hiddenTextId).children); 
    clearSentence()
    document.getElementById("page-info").innerText = pageInfoValue;
    $('#page-title').text(hiddenDivs[0].textContent.slice( 0, 100 ));

    hiddenDivs.forEach(hiddenDiv => {
      if (hiddenDiv.classList.contains("user")) {
        var question_html = '';
        ...
        question_html += hiddenDiv.textContent;
      ...
      } else if (hiddenDiv.classList.contains("assist")) {
        var response_html =  String(marked.parse(hiddenDiv.textContent));
        ...
        answer_html += '<button data-state="closed"><svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" class="cursor-pointer hover:opacity-50">'
        answer_html += '<path d="M4 12v-3a3 3 0 0 1 3 -3h13m-3 -3l3 3l-3 3"></path>'
        answer_html += '<path d="M20 12v3a3 3 0 0 1 -3 3h-13m3 3l-3 -3l3 -3"></path>'
        answer_html += '</svg></button></div></div>'
...
function clearSentence() {
  ...
  let qaInputs = document.getElementsByClassName("qa-user");
  while (qaInputs.length > 0) {
    qaInputs[0].remove();
  }
  
  let qaOutputs = document.getElementsByClassName("qa-assist");
  while (qaOutputs.length > 0) {
    qaOutputs[0].remove();
  }
...
function clearTitle() {
  document.getElementById("page-title").innerText = "New Chat";
}

function updPageInfo() {

  let childCount = document.getElementById("hidden-text").childElementCount;
  let pageInfoNum = childCount+1;
  document.getElementById("page-info").innerText = pageInfoNum;
}
